...
public abstract class AbstractSerialThread
{
    ...
    private void AttemptConnection()
    {
        ...
        //serialPort를 open하고나서 수동으로 Dtr,Rts 포트를 true로 설정하라는 얘기가 있어 수정.
    ...
    private void RunOnce()
    {
        try
        {
            ...
            int nbyte = serialPort.BytesToRead;                 //수신받은 데이터의 Byte수를 저장.
            string text = "";                                   //수신받은 Byte데이터들을 붙일 string선언.
            if (nbyte > 0)
            {
            ...
            if (text != "")
            {
                ...
                {
                    Debug.LogWarning("Queue is full. Dropping message: " + text);
                }
            }
        }
